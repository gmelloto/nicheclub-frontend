...
const COR_ACORDE = {
  ...
  'Frutado': '#e87878','Gourmet': '#e8a060','Couro': '#a07848',
  'Defumado': '#9898a8','Terroso': '#a08868','Herbal': '#90b870',
...
export default function Perfume() {
  ...
  useEffect(() => {
    window.scrollTo(0,0);
    api.perfume(id)
      .then(p => { setPerfume(p); return api.reservasPerfume(p.id).catch(()=>[]); })
  ...
  const PiramideBlock = () => (
    (notas_topo.length>0||notas_coracao.length>0||notas_base.length>0) ? (
      <div style={{background:C.bg2,border:'1px solid #e8e4dc',borderRadius:4,padding:'1.25rem',marginTop:'1rem'}}>
        <h3 style={{fontSize:'0.95rem',fontWeight:700,marginBottom:'1rem',color:C.text}}>Piramide Olfativa</h3>
        {notas_topo.length>0&&<div style={{marginBottom:'1rem'}}><span style={LBL}>Topo</span><div style={{display:'flex',flexWrap:'wrap',gap:8}}>{notas_topo.map(n=><NotaBadge key={n} nota={n} img={imgNota(n)}/>)}</div></div>}
        {notas_coracao.length>0&&<div style={{marginBottom:'1rem'}}><span style={LBL}>Coracao</span><div style={{display:'flex',flexWrap:'wrap',gap:8}}>{notas_coracao.map(n=><NotaBadge key={n} nota={n} img={imgNota(n)}/>)}</div></div>}
        {notas_base.length>0&&<div><span style={LBL}>Base</span><div style={{display:'flex',flexWrap:'wrap',gap:8}}>{notas_base.map(n=><NotaBadge key={n} nota={n} img={imgNota(n)}/>)}</div></div>}
      </div>
    ) : null
  );
              ...
              {perfume.rating_valor&&(
                <div style={{marginTop:'auto',paddingTop:8,borderTop:'1px solid #e8e4dc'}}>
                  <div style={{display:'flex',alignItems:'center',gap:4}}>
                    ...
                    <div style={{display:'flex',gap:1}}>{[1,2,3,4,5].map(s=><span key={s} style={{fontSize:10,color:s<=Math.round(perfume.rating_valor)?C.goldLight:C.border}}>&#9733;</span>)}</div>
                  </div>
        ...
        <div className="p-right">
          ...
          <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:'1rem'}}>
            {TAMANHOS.map(t=>{
              ...
              return(
                <button key={t.key} onClick={()=>{if(disp){setSelecionado(t.key);setMlAvulso('');}}} disabled={!disp}
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,width:68}}>
      <div style={{width:52,height:52,borderRadius:8,overflow:'hidden',border:'1px solid #e0d8c8',background:'#f0ede8',display:'flex',alignItems:'center',justifyContent:'center'}}>
        {img?<img src={img} alt={nota} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:20}}>&#127807;</span>}
      </div>
...
const DEMO = {
  ...
  opcoes:[
    {tamanho:'apc',preco:320,ml_quantidade:50},{tamanho:'3ml',preco:38,ml_quantidade:3},
    {tamanho:'5ml',preco:58,ml_quantidade:5},{tamanho:'10ml',preco:105,ml_quantidade:10},
