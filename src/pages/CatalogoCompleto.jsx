...
export default function CatalogoCompleto() {
  ...
  const carregar = async (pag = 1, termo = busca, reset = false) => {
    ...
    try {
      const res = await api.perfumes({ pagina: pag, limite: LIMITE, busca: termo, ordenar: 'nota' });
  ...
  const handleObserver = useCallback((entries) => {
    const [entry] = entries;
    if (entry.isIntersecting && temMais && !loading && !loadingMore) {
      carregar(pagina + 1, busca);
    }
  }, [temMais, loading, loadingMore, pagina, busca]);
    ...
    <div style={{ minHeight: '100vh', background: '#fff', color: '#111' }}>
      ...
      <div style={{ textAlign: 'center', padding: '2rem 2rem 3rem' }}>
        ...
        <p style={{ fontSize: 15, color: '#888', marginBottom: '2rem' }}>
          {total > 0 ? `${total} fragrâncias` : 'Explore nossa coleção completa!'}
        </p>
          ...
          <>
            ...
            {loadingMore && (
              <div style={{ textAlign: 'center', marginTop: '2rem' }}>
                <p style={{ fontSize: 13, color: '#aaa' }}>Carregando mais...</p>
              </div>
            )}

            {!temMais && perfumes.length > 0 && (
              <p style={{ textAlign: 'center', fontSize: 12, color: '#aaa', marginTop: '2rem' }}>
                {perfumes.length} de {total} fragrâncias
              </p>
            )}
          </>
        )}
